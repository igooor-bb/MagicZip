...
/// Creates archives with encrypted file contents, names and timestamps.
///
/// A shared password protects all files and the catalog. Entry count, compression methods
/// and approximate sizes remain visible.
///
/// - Important: This writer uses minizip-ng's CDCD extension and requires a compatible reader
///   such as ``SecureZIPReader``. It does not produce PKWARE encrypted catalogs.
///
...
public struct SecureZIPWriter: ~Copyable {
    ...
    ///   - modificationDate: The modification date to record in the archive.
    ///
    /// The directory name is protected by the encrypted catalog.
