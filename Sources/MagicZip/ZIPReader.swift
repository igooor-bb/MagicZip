...
public final class ZIPReader {
    ...
    public func read(
        path: String, password: String? = nil, chunkSize: Int = 64 * 1024, consumer: (Data) throws -> Void,
    ) throws {
        try operation {
            guard let entry = entry(at: path) else { throw ZIPError.entryNotFound(path) }
            var total: Int64 = 0
            try stream(entry, password: password, chunkSize: chunkSize, total: &total, consumer: consumer)
        }
    }
    ...
    public func data(path: String, password: String? = nil, maximumBytes: Int = 16 * 1024 * 1024) throws -> Data {
        guard maximumBytes >= 0 else { throw ZIPError.invalidArgument("maximumBytes must be nonnegative") }
        guard let entry = entry(at: path) else { throw ZIPError.entryNotFound(path) }
        guard entry.uncompressedSize <= maximumBytes else { throw ZIPError.limitExceeded(path) }
        var result = Data()
        try read(path: path, password: password) { chunk in
            guard chunk.count <= maximumBytes - result.count else { throw ZIPError.limitExceeded(path) }
            result.append(chunk)
        }
        return result
    }
    ...
    private func operation<T>(_ body: () throws -> T) throws -> T {
        ...
        defer { gate.unlock() }
        guard native.pointer != nil else { throw ZIPError.closed }
        return try body()
    }
    ...
    private func select(_ selection: ZIPSelection) throws -> [ZIPEntry] {
        ...
        case let .subtree(path):
            ...
            let selected = entries.filter { Data($0.path.utf8).starts(with: prefix) }
            guard !selected.isEmpty else { throw ZIPError.entryNotFound(path) }
            return selected
        }
    }

    private func stream(
        _ entry: ZIPEntry, password: String?, chunkSize: Int, total: inout Int64, consumer: (Data) throws -> Void,
    ) throws {
        guard (1 ... 1024 * 1024).contains(chunkSize) else { throw ZIPError.invalidArgument("chunkSize must be in 1...1048576") }
        ...
        guard entry.uncompressedSize <= limits.maximumTotalBytes - total else { throw ZIPError.limitExceeded(entry.path) }
        ...
        try withPassword(entry.encryption == .none ? nil : password) { password in
            ...
            try completing {
                ...
                while true {
                    ...
                    guard Int64(count) <= limits.maximumEntryBytes - produced,
                          Int64(count) <= limits.maximumTotalBytes - total,
                          Int64(count) <= entry.uncompressedSize - produced
                    else { throw ZIPError.limitExceeded(entry.path) }
                    produced += Int64(count)
                    total += Int64(count)
                    try consumer(Data(buffer.prefix(Int(count))))
                }
                verified = true
            } cleanup: {
    ...
    private static func scan(
        ...
        while status != -100 {
            ...
            guard let name = info.name else { throw ZIPError.unsafePath("") }
            ...
            guard bytes.count <= limits.maximumPathBytes - pathBytes else { throw ZIPError.limitExceeded("Entry-name bytes") }
            ...
            else { throw ZIPError.unsupported(path: path, feature: "Symlink or special file") }
            guard info.disk == 0 else { throw ZIPError.unsupported(path: path, feature: "Split archive") }
            ...
            guard info.uncompressed_size <= limits.maximumEntryBytes,
                  Double(info.uncompressed_size) / Double(max(1, info.compressed_size)) <= limits.maximumCompressionRatio
            else { throw ZIPError.limitExceeded(path) }
            guard !directory || info.uncompressed_size == 0 else { throw ZIPError.conflictingPath(path) }
            ...
            entries.append(ZIPEntry(
                path: path, isDirectory: directory, compressedSize: info.compressed_size, uncompressedSize: info.uncompressed_size,
                modificationDate: Date(timeIntervalSince1970: TimeInterval(info.modified)), compressionMethod: info.method,
                encryption: encryption, crc32: info.crc, position: info.position,
            ))
            status = magiczip_next(pointer)
        }
        guard entries.count == count else { throw ZIPError.backend(operation: "validate entry count", path: nil, status: -103) }
        return entries
    }
}
