...
public struct ZIPEntry: Sendable, Equatable {
    /// The exact UTF-8 archive path; directories have a trailing slash.
...
public struct ZIPLimits: Sendable {
    ...
    func validate() throws {
        guard maximumEntries >= 0, maximumPathBytes >= 0, maximumEntryBytes >= 0,
              maximumTotalBytes >= 0, maximumCompressionRatio.isFinite, maximumCompressionRatio > 0
        else { throw ZIPError.invalidArgument("Resource limits must be finite and nonnegative; ratio must be positive") }
    }
}
